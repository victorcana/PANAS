...
function helpPanel(){
	...
	echo -e "\t-h:\t Help"
	echo -e "Example:\t ~/path/SANSRAPI.sh -p protein_directory/ -n nucleotide_directory/ -o Orthologs/ -z 4 -a mafft -g 1 -t OMA -s single-copy"
	tput cnorm; exit 1
}
#####Main Function
...
###Step to filter and order the information generated in the analysis.
	...
	paste borrar Resultados.$i.vic |sed 's/output\///g' | awk  -F"\t"   '{print $1"\t"$2"\t"$2"\t"$4"\t"$6"\t"$8"\t'"$single_multiple"' OGs"}'| sed 's/-/\t/ ; s/-/\t/ ; s/]/-/g'|  sed  '1i ID_1\tID_2\tSpecies_1\tSpecies_2\tPairwiseOrthologs\tdN_dS\tdN\tdS\tHOG' > Resultados.$i.vic2 
	rm borrar
	cd ..;
done
...
cat *carpeta/*.vic2 | grep -v 'dN_dS'| sed  '1i ID_1\tID_2\tSpecies_1\tSpecies_2\tPairwiseOrthologs\tdN_dS\tdN\tdS\tHOG' > ../Todos_resultados_$single_multiple
}
