	...
	if [ "$(echo $type)" == "OMA" ]; then
		if [ "$(echo $single_multiple)" == "single-copy" ]; then
			...
			if [ "$(echo $single_multiple)" == "multiple-copy" ]; then
				...
				echo "correctly specify parameter -s"
				exit 1;
			fi
		fi
	else
...
###Step to filter and order the information generated in the analysis.
	tail -q -n1 output/*mlc | grep  "dN/dS"  | sed 's/ S=/\tS=\t/g' | sed 's/ N=/\tN=\t/g'|sed 's/ dN\/dS=/\tdN\/dS=\t/g'| sed 's/ dN =/\tdN=\t/g'   |sed 's/ dS =/\tdS=\t/g' | sed 's/t=/'"$i"'\t/g' | cut -f 1,7,8,9,10,11,12 > Results.$i.vic  
	...
	paste borrar Results.$i.vic |sed 's/output\///g' | awk  -F"\t"   '{print $1"\t"$2"\t"$2"\t"$4"\t"$6"\t"$8"\t'"$single_multiple"' OGs"}'| sed 's/-/\t/ ; s/-/\t/ ; s/]/-/g'|  sed  '1i ID_1\tID_2\tSpecies_1\tSpecies_2\tPairwiseOrthologs\tKa_Ks\tKa\tKs\tHOG' > Results.$i.vic2 
	rm borrar
	cd ..;
done
...
cat *carpeta/*.vic2 | grep -v 'Ka_Ks'| sed  '1i ID_1\tID_2\tSpecies_1\tSpecies_2\tPairwiseOrthologs\tKa_Ks\tKa\tKs\tHOG' > ../All_results_$single_multiple
}
